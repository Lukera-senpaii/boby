...
/*👆API KEY EX TEAM👆 adquira a sua key com*/ o = 'wa.me/+'
...
//  tiringa.logger.level = 'trace'
    ...
    tiringa.on('qr', () => {
        console.log(color('👆'), color(' Escanear o código acima para iniciar o Boteco-BOT!'))
    })
    fs.existsSync('./tiringa.json') && tiringa.loadAuthInfo('./tiringa.json')
    tiringa.on('connecting', () => {
        start('2', 'Conectando o Boteco-BOT...')
    })
    tiringa.on('open', () => {
            success('2', 'Boteco-BOT conectado!!!')
        })
  ...
  tiringa.logger.error('Reconectando...')
    ...
    tiringa.on('chat-update', async(mek) => {
        try {
            ...
            const ownerNumber = [`5516991766933@s.whatsapp.net`, `557499896089@s.whatsapp.net`]
            const liaN = [`5511962163969@s.whatsapp.net`]
            const ownerNumberB = [`5516991766933@s.whatsapp.net`]
          ...
          /*      if (testat === "fdp") {
              ...
              if (testat.match('tiringa')) {
                ...
                if (budy.match(`@557499896089`)) {
                  ...
                  if (budy.match(`@5516991766933`)) {
                    sn = ["1", "2", "3"]
                    res = sn[Math.floor(Math.random() * sn.length)]
               /*     if (res == "1") {
                        result = fs.readFileSync(`./src/Lukera.mp3`)
              ...
              //   await dappauhuy(dapuhy, MessageType.text, tescuk, `${ucapanFakereply}`)
               ...
               case 'x9':
                ...
                if (mentioned.length !== 0){
                    ...
                    if (quotedMsg.sender.match('5516991766933')) return reply(`🤨`)
...
e upar seu level
  ═════◄••LEVEL••►═════`*/
                ...
                addFilter(from)
                    if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                        ...
                        await ffmpeg(`./${media}`)
                            ...
                            .on('end', function() {
                                console.log('Finish')
                                exec(`webpmux -set exif ${addMetadata('Tiringa-BOT', 'Lukera Dev')} ${ran} -o ${ran}`, async(error) => {
                ...
                addFilter(from)
                    if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                        ...
                        exec(`ffmpeg -i ${media} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 800:800 ${rano}`, (err) => {
                            fs.unlinkSync(media)
                            exec(`webpmux -set exif ${addMetadata('Tiringa-BOT', 'Lukera Dev')} ${rano} -o ${rano}`, async(error) => {
                ...
                case 'bugreport':
                    ...
                    tiringa.sendMessage('5516991766933@s.whatsapp.net', options, text, {
                        quoted: mek
                    })
                ...
                case 'hora':
                    ...
                    //_GIT DO BOT
                case 'git':
                    tiringa.sendMessage(from, `https://github.com/LukeraH/Tiringa-BOT`, text, {
                        quoted: mek
                    })
                    addFilter(from)
                    break
                ...
                case 'pão':
                    if (isOwner) return reply('Oi Lukera')
                    else {
...
`
  ...
  *◍₊ิ۪͡୭⇾Total de membros: ${participants.length} membros`
                ...
                case 'nomeninja':
                    addFilter(from)
                    if (args.length < 1) return reply(`Exemplo: ${prefix}nomeninja Lukera`)
                    tiringa.updatePresence(from, Presence.composing)
                    nome = body.slice(10)
                    try {
...
você se registrou, digite ${prefix}menu para listar meus comandos`
                ...
                case 'hartatahta':
                    if (args.length < 1) return reply(`Exemplo: ${prefix}hartatahta El Lukera`)
