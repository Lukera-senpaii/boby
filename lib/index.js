...
const prefix = '/'
const botName = "Boteco-BOT"
const ownerNumber = "5516991766933"
const ownerName = "Lukera"
...
tiringa.on('group-participants-update', async (anu) => {
      ...
      tiringa.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = [`5516991766933@s.whatsapp.net`]
			const ownerNumberB = []
			//const ban = [`5516991766933@s.whatsapp.net`]
...
tiringa.sendMessage(from, `https://github.com/LukeraH/Tiringa-BOT` , text, {quoted: mek})
break
...
//_ENVIA O VCARD
case 'Lukera':
const sentMsg  = await tiringa.sendMessage(from, {displayname: "Lukera🧙‍♂️", vcard: vcard}, MessageType.contact)
break
                    case 'leaderboard':
				case 'lb':
...
case 'owner':
msgFilter.isFiltered(from)
Lukeraft = fs.readFileSync('./teste/gojo.jpg')
tiringa.sendMessage(from, Lukeraft, image, {quoted: mek, caption: `☆━✪🕴  ∴₰Ⱦꪋℓo፝֯֟ ߷  🕴✪━☆\n\nChat do Lukera: wa.me/+5516991766933`})
break
...
reply('Procurando sua música...⏳')
  ...
  aanu = await fetchJson(`https://api-tiringa.Lukerah.repl.co/api/yta?url=${anu.result.source}`)
  infomp3 = 
`    MÚSICA ENCONTRADA
...
msgFilter.isFiltered(from)
  ...
  try {
  anu = await fetchJson(`https://api-tiringa.Lukerah.repl.co/api/yta?url=${play}`)
...
//_LIMPA TODOS OS CHATS
				case 'clearall':
					if (!isOwner) return reply('Só o Lukera pode fazer isso')
